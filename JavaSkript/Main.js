...
function load() {
	...
	darkMode.onclick = function () {
		...
		if (isStorageAvailable) localStorage.setItem("dark", darkModeBool);
	};
	...
	var modalClose = document.getElementById("closeModal");
	var captionText = document.getElementById("captionModal");
	for (let element of document.getElementsByClassName("imageSection")) {
		for (let div of element.children) {
			div.children[0].onclick = function () {
				...
				if (isMobile) body.style.padding = "0";
				else body.style.padding = "0 17px 0 0";
			};
		}
	}
...
function toggleDarkModeIcon(toggle = true) {
	...
	if (toggle) darkModeText.textContent = "Light Mode";
